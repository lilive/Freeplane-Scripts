...
class ChangeListener implements INodeChangeListener, IMapChangeListener
{
	...
	{
		// When a node is inserted, we must take car about its named-bookmark
		// icon, because it can be a copy of another node named-bookmarked.
		// But only one node can be named-bookmarked with a single keyboard key,// so we must remove the named-bookmark icon from the copy.
